...
(() => {
    ...
    const addBtn = document.querySelectorAll<HTMLButtonElement>(".buy-btn");
    const delBtn = document.querySelectorAll<HTMLButtonElement>(".delete-btn");
    
    function addCart(item: HTMLButtonElement): void {
      const gotcha = getElId(item);
      if (!item.parentElement) return
      if (!gotcha || indexInCart(gotcha) !== -1) return
      cart.push(gotcha);
    ...
    function indexInCart(el: menuItem): number {
    ...
    function findAllBtnPairs(item: HTMLElement): void {     
      const allBtns = item.querySelectorAll<HTMLButtonElement>("button[data-action='add'], button[data-action='delete']");
